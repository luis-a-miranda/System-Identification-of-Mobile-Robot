function [G_levy, J, handle] = levy (W_cut, GAIN_cut, PHASE_cut, Q, weights)
...
coefficients = MAT \ b;
G_levy.num = (coefficients(end:-1:n+1)).'; % ...again the increasing/decreasing order thing
G_levy.den = [coefficients(n:-1:1); 1]';

% index J
if nargout > 1 % quadratic error was required
    tempNum = 0;
    for c = 1:length(G_levy.num)
        tempNum = tempNum + G_levy.num(c) * (1i*W_cut).^(Q.num(m-c+2)); % strage way to match G and Q... because of the increasing/decreasing order thing again!
    end
    tempDen = 0;
    for c = 1:length(G_levy.den)
        tempDen = tempDen + G_levy.den(c) * (1i*W_cut).^(Q.den(n-c+2));
    end
...
if nargout == 3 % a Bode plot was required
    ...
    tempNum = 0;
    for c = 1:length(G_levy.num)
        tempNum = tempNum + G_levy.num(c) * (1i*wNew).^(Q.num(m-c+2));
    end
    tempDen = 0;
    for c = 1:length(G_levy.den)
        tempDen = tempDen + G_levy.den(c) * (1i*wNew).^(Q.den(n-c+2));
    end
